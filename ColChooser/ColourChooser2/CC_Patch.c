...
struct Patch * CreatePatch( int x, int y, int w, int h, int pid, RECT *bounds, unsigned int col, enum Shape shape )
{
    struct Patch *newp;
    ...
    newp->pos.bottom = newp->pos.top + h;
...
//brilliant use of recursion, if I may say so myself
...
unsigned int GetColour( struct Patch *patch )
{
    return patch->col;
}


void SetColour( struct Patch *patch, unsigned int col )
{
    patch->col = col;
}
